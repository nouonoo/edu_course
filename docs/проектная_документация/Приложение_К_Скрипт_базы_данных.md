# Приложение К. Скрипт создания базы данных

**Проект:** Адаптационный курс для сотрудников ритуальной компании  
**СУБД:** Microsoft SQL Server  
**База данных:** `LearningPlatformDB`  
**Версия:** 1.1  
**Дата:** июнь 2026
...
Документ содержит актуальный SQL-скрипт развёртывания базы данных LMS. Скрипт создаёт базу, девять таблиц, внешние ключи, уникальные ограничения и начальные тестовые данные.

Идентичная копия скрипта в репозитории: `backend/LearningPlatformDB.sql`.

Для обновления **уже существующей** базы без пересоздания используйте `backend/migration.sql`.
...
| 2 | `Roles` | Справочник ролей |
| 3 | `Users` | Учётные записи сотрудников |
| 4 | `UserRoles` | Связь пользователь ↔ роль (M:N) |
| 5 | `Courses` | Каталог электронных курсов |
| 6 | `Assignments` | Назначения курсов сотрудникам |
| 7 | `Section_progress` | Прогресс по разделам в рамках назначения |
| 8 | `User_result` | Итоговые баллы по курсу |
| 9 | `Feedback` | Обратная связь (зарезервировано в шаблоне БД) |

---

## 3. Порядок выполнения

1. Запустить службу SQL Server.
2. Открыть SSMS, подключиться к экземпляру (`localhost`, `.\SQLEXPRESS` или `(localdb)\MSSQLLocalDB`).
3. Выполнить скрипт из раздела 4 **целиком**.
4. Проверить строку подключения в `backend/config.py` (ODBC Driver 17, имя сервера, база `LearningPlatformDB`).
5. Запустить `py -3 backend/test_db.py` для проверки соединения.
...
/* --- Миграция столбцов для старых версий --- */
...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_Users_Position')
    ALTER TABLE [dbo].[Users] ADD CONSTRAINT [FK_Users_Position] FOREIGN KEY([position_id]) REFERENCES [dbo].[Position]([Position_id]);
GO
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_UserRoles_Users')
    ALTER TABLE [dbo].[UserRoles] ADD CONSTRAINT [FK_UserRoles_Users] FOREIGN KEY([user_id]) REFERENCES [dbo].[Users]([user_id]) ON DELETE CASCADE;
GO
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_UserRoles_Roles')
    ALTER TABLE [dbo].[UserRoles] ADD CONSTRAINT [FK_UserRoles_Roles] FOREIGN KEY([role_id]) REFERENCES [dbo].[Roles]([role_id]) ON DELETE CASCADE;
GO
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_Courses_Author')
    ALTER TABLE [dbo].[Courses] ADD CONSTRAINT [FK_Courses_Author] FOREIGN KEY([author_id]) REFERENCES [dbo].[Users]([user_id]);
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_UserResult_Users')
    ALTER TABLE [dbo].[User_result] ADD CONSTRAINT [FK_UserResult_Users] FOREIGN KEY([user_id]) REFERENCES [dbo].[Users]([user_id]) ON DELETE CASCADE;
GO
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_UserResult_Courses')
    ALTER TABLE [dbo].[User_result] ADD CONSTRAINT [FK_UserResult_Courses] FOREIGN KEY([course_id]) REFERENCES [dbo].[Courses]([course_id]) ON DELETE CASCADE;
GO
...
BEGIN
    ...
    INSERT INTO [dbo].[Position] ([Position_id], [name]) VALUES (1002, N'Разработчик'), (1003, N'Сис админ');
...
BEGIN
    ...
    INSERT INTO [dbo].[Roles] ([role_id], [role_name]) VALUES
        (1, N'Разработчик'), (2, N'Эксперт'), (3, N'Обучающийся'), (4, N'Администратор');
...
BEGIN
    ...
    INSERT INTO [dbo].[Users] ([user_id], [name], [surname], [patronymic], [position_id], [email], [password_hash], [photo], [birthday], [status])
    VALUES
...
BEGIN
    ...
    INSERT INTO [dbo].[UserRoles] ([UserRoles_id], [user_id], [role_id]) VALUES (2, 3, 2), (3, 6, 3), (4, 8, 3), (5, 12, 4);
...
BEGIN
    ...
    INSERT INTO [dbo].[Courses] ([course_id], [title], [description], [author_id], [date], [storage])
    VALUES (2, N'Безопасность', N'Курс по информационной безопасности', 3, CAST(N'2024-11-11T00:00:00.0000000' AS datetime2), N'bezopasnost');
    SET IDENTITY_INSERT [dbo].[Courses] OFF;
END
GO

UPDATE [dbo].[Courses] SET [storage] = N'bezopasnost', [description] = N'Курс по информационной безопасности'
WHERE [course_id] = 2 AND ([storage] IS NULL OR [storage] <> N'bezopasnost');
GO
...
| Эксперт | `miereeva@yandex.ru` | `mashamasha` |
| Обучающийся | `pkabkov@ra.ru` | `petya` |
| Обучающийся | `vor.rr@va.ru` | `22222` |

---

## Примечание для переноса в Word

Вставьте документ как **«Приложение К. Скрипт создания базы данных»**. SQL-код оформите моноширинным шрифтом (Courier New, 9 pt).
