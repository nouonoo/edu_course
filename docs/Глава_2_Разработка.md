...
На основе полученной на этапе проектирования ERD-модели была разработана база данных с помощью средств MS SQL Server. Скрипты `backend/dump.sql` и `backend/migration.sql` создают базу `LearningPlatformDB`, начальные данные и таблицы назначений. Логическая схема представлена в **Приложении Е**; физическая ER-диаграмма приведена ниже.
...
Формирование Excel-отчёта выполняется функцией `create_excel_report()` с использованием библиотек **pandas** и **openpyxl**: в файл выгружаются ФИО, статус прохождения и дата.
...
Столбцы отчёта: ФИО, статус прохождения (Успешно / Неуспешно относительно порога 70 %), дата прохождения.
